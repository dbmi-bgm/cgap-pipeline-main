...
CGAP Pipeline Upstream GATK is our open source option for processing Whole Genome Sequencing (WGS) and Whole Exome Sequencing (WES) datasets.  This pipeline begins with ``fastq`` files and produces ``bam`` files (mapped reads) which can be supplied to numerous CGAP downstream pipelines, including the SNV Germline and SV Germline Pipelines. If the user wishes to provide ``cram`` files as input, these can first be converted to ``fastq`` files using the CGAP Pipeline Base module.
...
Both the WES and WGS configurations of the CGAP Pipeline Upstream GATK are mostly based on ``bwa`` and ``gatk4``. Users can select the correct pipeline configuration for their input data through the use of the corresponding metaworkflows (e.g., ``CGAP_WES_proband_only.json`` for a single sample of WES data or ``CGAP_WGS_trio_60x.json`` for a trio (mother, father, and proband) sequenced deeply following WGS library preparation).
