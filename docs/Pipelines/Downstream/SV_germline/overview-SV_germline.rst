====================================
Overview - CGAP Pipeline SV Germline
====================================

CGAP Pipeline SV Germline (https://github.com/dbmi-bgm/cgap-pipeline-SV-germline) identifies, annotates, and filters structural variants (SVs) starting from short read sequencing alignment files (``bam``) and produces ``vcf`` files as output.  SVs are a class of large genomic variants that includes deletions, duplications, translocations, inversions and other complex events, generally with a size of 50 bp or longer.
...
The CGAP Pipeline SV Germline is designed for proband-only or trio analysis, with the proband diagnosed with a likely monogenic disease. It receives the initial ``bam`` file(s) from either of the `WGS Upstream pipelines <https://cgap-pipeline-master.readthedocs.io/en/latest/Pipelines/Upstream/Upstream_pipelines.html>`_.

**Note**: CGAP Pipeline SV Germline is not optimized for Whole Exome Sequencing (WES) data.
...
Largely, the pipeline consists of three parts:
  ...
  | Part 1. Beginning from ``bam`` file(s) generated by either of the available ``upstream pipelines`` of the `WGS Upstream pipelines <https://cgap-pipeline-master.readthedocs.io/en/latest/Pipelines/Upstream/Upstream_pipelines.html>`_, SVs are identified using ``Manta``
