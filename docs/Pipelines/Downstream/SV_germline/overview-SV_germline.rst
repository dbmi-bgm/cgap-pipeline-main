...
Largely, the pipeline consists of three parts:
  ...
  | Part 1. Beginning from ``bam`` file(s) generated by either of the available ``upstream pipelines`` of the `WGS Upstream pipelines <https://cgap-pipeline-master.readthedocs.io/en/latest/Pipelines/Upstream/Upstream_pipelines.html>`_, or provided by the user, SVs are identified using ``Manta``
