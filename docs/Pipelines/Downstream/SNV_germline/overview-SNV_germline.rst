...
The CGAP Pipeline for Germline Single Nucleotide Variants (SNVs) (https://github.com/dbmi-bgm/cgap-pipeline-SNV-germline) processes Whole Genome Sequencing (WGS) and Whole Exome Sequencing (WES) data starting from ``bam`` files, and produces ``g.vcf`` and ``vcf`` files containing SNVs and small INDELs as output.

The CGAP Pipeline SNV Germline supports ``bam`` files generated by mapping raw reads from both WGS and WES sequencing runs to **hg38**. The WGS configuration is designed for a trio analysis with proband diagnosed with a likely monogenic disease. It is optimized for data with 30x coverage and has been tested with data up to 80-90x coverage. It can also be run in proband-only, and family modes using appropriate metaworkflows.
