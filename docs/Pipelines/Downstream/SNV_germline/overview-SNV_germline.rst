...
It can receive the initial ``bam`` file(s) from either of the `WGS Upstream pipelines <https://cgap-pipeline-master.readthedocs.io/en/latest/Pipelines/Upstream/Upstream_pipelines.html>`_.
...
The CGAP Pipeline SNV Germline is primarily used for variant calling following GATK (Genome Analysis Toolkit) Best Practice. Variants are then annotated and filtered.
