...
CGAP Pipeline SNV Germline processes Whole Genome Sequencing (WGS) and Whole Exome Sequencing (WES) data starting from ``bam`` files and produces ``g.vcf`` and ``vcf`` files containing variant calls as output.
